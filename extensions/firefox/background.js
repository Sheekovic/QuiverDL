...
async function settings() {
  return api.storage.local.get({ token: "", interceptionEnabled: false, minimumBytes: 50 * 1024 * 1024, allowedDomains: [] });
}

async function enqueue(url, suggestedFilename) {
  const current = await settings();
  if (!current.token) throw new Error("Pairing token is not configured");
  return api.runtime.sendNativeMessage(HOST, { version: 1, action: "enqueue", token: current.token, url, suggestedFilename: suggestedFilename || null });
}

api.contextMenus.onClicked.addListener((info) => {
  if (info.menuItemId === "quiverdl-download" && info.linkUrl) void enqueue(info.linkUrl, null).catch(console.error);
});
