...
package entries on the release branch before CI starts. The synchronizer always executes from the
immutable triggering `main` commit without push credentials; the credential is exposed only to the
final hook-disabled push. Linux then uses locked Cargo and npm dependency graphs,
